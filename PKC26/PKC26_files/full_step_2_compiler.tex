...
compiled using Step 1 can be re-casted, under certain assumption, as SNARKs for a new type of NP relation 
...
Let $\mathscr{P}_{\mathit{vec}}$ be a ranged polynomial protocol for relation $\mathcal{R}_{\mathit{vec}}^c$. 
...
Since $c$ applies only to a part of the public input for relation $\mathcal{R}_{\mathit{vec}}^c$ 
(i.e., $\mathbf{input_1}$), we can apply Lemma~\ref{le:compilation_step_1} of Appendix~\ref{supplementary_poly_protocols_appendix} and Step 1 
of our compiler to polynomial protocol $\mathscr{P}_{\mathit{vec}}$. We denote by $\mathscr{P}^*_{\mathit{vec}}$ the hybrid model SNARK obtained 
after compiling $\mathscr{P}_{\mathit{vec}}$ using compilation Step 1. Due to (HMA.1.) and according to Step 1, the SNARK verifier in 
...
by $\mathcal{C}$ the set of all $\mathit{KZG}$ poly commitments or vectors of such poly commitments.\\

We next define the relation: 
\begin{align*}
...
\noindent Note that $\mathcal{R}_{\mathit{vec}}^c$ and $\mathcal{R}_{\mathit{vec}, \mathit{com}}^c$ differ in three aspects: \\
1) $\mathbf{input_1}$ for $\mathcal{R}_{\mathit{vec}}^c$ has been renamed $\mathbf{witness_2}$ for $\mathcal{R}_{\mathit{vec}, \mathit{com}}^c$;  \\
2) predicate $\mathbf{C} = \mathbf{Com}(\mathbf{witness_2})$ has been added to $\mathcal{R}_{\mathit{vec}, \mathit{com}}^c$;  \\
3) $\mathbf{input_1}$ for $\mathcal{R}_{\mathit{vec}}^c$ has been replaced by $\mathbf{C}$ as an input for $\mathcal{R}_{\mathit{vec}, \mathit{com}}^c$.  \\

\noindent Finally, for $\mathbf{witness_2} = \mathbf{input_1}$ part of $\mathit{state_1}$, we define $\mathit{SNARK.PartInput}$:
\begin{align*} 
...
& \ \ \ \ \ \ \ \ \ \mathit{Return} ( \mathbf{C}, \mathit{state_2})
\end{align*}
...
\begin{itemize}
\item $\mathit{SNARK.Setup}$ and $\mathit{SNARK.KeyGen}$ are identical to the corresponding algorithms for $\mathscr{P}^*_{\mathit{vec}}$ for 
relation $\mathcal{R}_{\mathit{vec}}^c$ as compiled in Step 1. 
\item $\mathit{SNARK.PartInput}$ has been described three lines above. 
\item $\mathit{SNARK.Prover}$ is identical to the corresponding algorithm for $\mathscr{P}^*_{\mathit{vec}}$ with the appropriate 
re-interpretation of the public inputs and witness, i.e., the algorithm remains the same but the witness and public input 
become those for $\mathcal{R}_{\mathit{vec}, \mathit{com}}^c$.
\item $\mathit{SNARK.Verifier}$ is identical to the corresponding algorithm for $\mathscr{P}^*_{\mathit{vec}}$ with the appropriate 
...
Let $\mathcal{A}$ be an AGM adversary against knowledge soundness in the hybrid model w.r.t. 
...
these two predicates hold: $$p_1(\mathbf{input_1}, \mathbf{input_2}, \mathbf{witness_1}) = 1, 
p_2(\mathbf{input_1}, \mathbf{input_2}, \mathbf{witness_1}) = 1.$$ If we let 
...
using $\mathcal{E}_{\mathit{KZG}}$ and $\mathcal{E}$ we can build an extractor for any knowledge-soundness AGM adversary $\mathcal{A}$ for alleged 
