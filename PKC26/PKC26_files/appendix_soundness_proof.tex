...
\begingroup
\renewcommand{\thetheorem}{\ref{th:soundness}}
\begin{theorem} 
...
then, together with the assumptions stated at the beginning of Appendix~\ref{sec:LCinstantiation} and for 
$\mathcal{R} \in \{ \Rlacom, \Racom \}$, the tuple ($\mathit{LC.Setup}$, $\mathit{LC.GenerateProof}$, $\mathit{LC.VerifyProof}$) 
as instantiated in Appendix~\ref{sec:LCinstantiation} is a light client  as formalised by Definition~\ref{scheme_light_client}.
\end{theorem}
