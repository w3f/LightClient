...
\noindent{\color{red} Next, we introduce the following Lagrange interpolation polynomials of degree at most $n-1 = |H|$ over the cyclic group $H$ (see Section~\ref{sec:lagrange}): 
$b(X)$ - interpolates the bits of the bitvector $\mathit{bit}$; $pkx(X)$, $pky(X)$ - interpolate all public keys' 
$x$ and $y$ coordinates, respectively; $kaccx(X)$, $kaccy(X)$ - interpolate the $x$ and $y$ coordinates, respectively, 
of the iterative sum of the actual signing validators' public keys. We also define five polynomial identities $id_1(X), \ldots, id_5(X)$ supporting the following intuition: $id_1(X)$, $id_2(X)$ 
ensure the $x$ and, respectively, the $y$ coordinates of the partial sum of actual signing validators public keys follows formulas $(\ast)$, $(\ast\ast)$ from Observation 1; $id_3(X)$, $id_4(X)$ 
ensure first partial aggregate sum is $h$ and the total sum is $h + \mathit{apk}$; this is necessary in order to ensure the addition of the public keys (which are elliptic curve points) 
never falls into case c) defined in Observation 1; $id_5(X)$ ensures $b(X)$ evaluates to bits over $H$. Together, polynomial identities $id_1(X)$ to $id_4(X)$ are necessary 
to define the corresponding $H$-ranged polynomial protocol $\Pla$ for relation $\Rla$. In more detail, we have:}  \\
