...
which is a generalisation for zero-knowledge proofs/arguments in which the prover proves statements about values that are committed. We use this paradigm by using a commitment to a large dataset in the input, in a similar way to \cite{HP_paper,CP_paper}. Other CP SNARK protocols (\cite{CP_paper,interoperability_2, lunar, eclipse} consider having (ZK) SNARKs with witness data in common, a very different problem to ours. Hash-and-prove~\cite{HP_paper} considers using hashes of a large input dataset in the input for a SNARK. LegoSNARKs ~\cite{CP_paper} consider using commitments which work with the SNARK protocols more directly and don't need an opening to be explicitly verified inside the SNARK. \cite{CP_paper} claims a 5000x speedup in proving time using this technique for Groth16 in place of hasing.  Our scheme uses a version of this approach, using KZG polymomial commitments~\cite{KZG_10} for the input, and we expect that several orders of magnitude for our proving time improvment over Celo's Plumo protocol that uses hashes comes from this, because we also have a very simple constraint system that would be much bigger if we hashed the input. Our formalism, which generalised PLONK~\cite{plonk}'s polynomial protocols and compiler to work with polynomial commitments for the input, is different from prior work on CP SNARKs.
