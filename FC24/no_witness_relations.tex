...
we review it and define a refinement of it in Section~\ref{supplementary_poly_protocols_appendix};  
...
(obtained using our two-step compiler) and our instantiation for BLS aggregatable signatures from Section~\ref{sec:bls}. 
...
id_1(X) = & (X-\omega^{n-1}) \cdot [b(X) \cdot ((kaccx(X)-pkx(X))^2 \cdot (kaccx(X)+ pkx(X) + 
 kaccx(\omega\cdot X)) - (pky(X) - kaccy(X))^2) + \\ 
...
id_2(X)  =  & (X-\omega^{n-1})\cdot [b(X) \cdot ((kaccx(X) - pkx(X)) \cdot (kaccy(\omega \cdot X) + kaccy(X)) - 
 ...
 & + (1-b(X)) \cdot   \cdot(kaccx(\omega \cdot X) - kaccx(X))]. \\
...
in accordance to Definition~\ref{def_ranged_poly_protocol} from Section~\ref{supplementary_poly_protocols_appendix}. \\
\vspace{-0.15in}
...
$\Rlacom$ defined below and $\Racom$ defined in Section~\ref{compiler_step_2}.
...
%\end{align*}
The two-step compiler is described in Section~\ref{sec_two_step_compiler}, with the first 
