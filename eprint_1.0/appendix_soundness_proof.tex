\section{Appendix D - Postponed Security Proof for Light Client Soundness}
...
\begin{itemize}
\item Statement $A(j)$ (for every $j < i$): assuming further that $\mathit{com_j} = \mathbf{Com}(\mathbf{pk_j})$, %and that the validity of the proofs of 
