...
\end{itemize}
\noindent that fulfils perfect completeness, succinctness and adaptive knowledge soundness as defined below.
...
and every $x \in \mathcal{L}_{\mathcal{R}}$, we have: \\
\noindent $\mathsf{Gen}$ runs in $\mathsf{poly}(\lambda)$ time, $\mathsf{Prove}$ runs in $\mathsf{poly}(\lambda + |x|)$ time; 
$\mathsf{Verify}$ runs in $\mathsf{poly}({\color{red} \ldots})$ time and an honestly generated proof has size ${\color{red} \ldots}$. 
\paragraph{Adaptive proof of knowledge for $\mathbb{O}$:} Consider the following experiment for security parameter $\lambda \in \mathbb{N}$, 
adversary $\mathcal{A}$, extractor $\mathcal{E}_{\mathcal{A}}$, auxiliary input generator $\mathcal{Z}$ and oracle family $\mathbb{O}$:\\
\begin{align*}
...
$\mathcal{A}$ during its execution. $\Pi$ satisfies adaptive proof of knowledge with respect to oracle family  $\mathbb{O}$ and auxiliary 
input from $\mathcal{Z}$ if for every non-uniform oracle prover $\mathcal{A}^{\mathcal{O}}$ of sizes $s(\lambda)= \mathsf{poly}(\lambda)$ 
making at most $Q(\lambda) = \mathsf{poly}(\lambda)$ queries there exists a non-uniform extractor $\mathcal{E}_{\mathcal{A}}$ 
of size $t(\lambda)=\mathsf{poly}(\lambda)$ and a negligible function $\mathsf{negl}(\lambda)$ such that 
