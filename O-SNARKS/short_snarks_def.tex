...
Let $\mathbb{G}$ be a cyclic group of prime order $p$ and let $\mathit{AlgAdv}_{\mathbb{G}}$ be the set of all algebraic 
adversaries with respect to $\mathbb{G}$. We say that a family of oracles $\mathbb{O}$ is AGM respecting relative to 
$\mathbb{G}$ if the probability of any oracle $\mathcal{O} \in \mathbb{O}$ outputting an element of $\mathbb{G}$ or outputting 
an element which can be transformed via an efficient algorithm into an element of $\mathbb{G}$ is $0$. 
\end{definition}
...
as per Definition~\ref{def:agm_oracles}. If $\Pi$ is a $\mathcal{Z}$-auxiliary input SNARK secure in the AGM model for any benign 
