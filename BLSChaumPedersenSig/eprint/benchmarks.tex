...
The benchmarks in Table~\ref{tab:aggregated_signature_bench} demonstrate that when the number of signers increases, the advantage of performing the aggregation of public keys in $\GoneBLS$ becomes more prominent. This result is expected because the scheme presented in this work performs only two extra scalar multiplications in $\GoneBLS$ compared to standard BLS verification with aggregate public key in $\GtwoBLS$. %However, when the number of signers increases, so does the number of elliptic curve additions performed in the faster $\GoneBLS$ group compared to operations in the slower $\GtwoBLS$ %(performed by standard BLS verification). 
