...
$$\mathcal{R}^c = \{(x;w) : (p_1(x,w) =1 \ | \ c(x,w) =1) \}$$ and we interpret it as the NP relation containing the pairs of inputs and witnesses 
$(x,w)$ such that $c(x,w) =1$, $p_1(x,w) = 1$ hold. However, to prove that $(x,w) \in \mathcal{R}^c$ we assume/take it as a given that 
$c(x,w) =1$ and we are left to prove only that $p_1(x,w) = 1$ holds. The reason we separate predicate $c(x,w)$ from predicate $p_1(x,w)$ in the definition 
...
of public input and we assume that inputs are parsed by honest parties without requiring additional checks. 
%We assume that all our relations have been generated using the security parameter $\lambda$. 
It is clear that a regular NP relation can be interpreted as a conditional relation $\mathcal{R}^c$, where $c$ is the predicate that always outputs $1$.
\vspace{-0.07in}
