...
which may be malicious. This is because our light client system inherits the property of asynchronous safety from the Byzantine agreement
...
In Appendix~\ref{sec:missing_snark_proofs} we give the postponed proofs related to the packed accountable protocol from Section~\ref{sec_a}. 
...
We assume that $\mathit{info}_n$ can be proven from a commitment to the state at block number $n$ that is signed by validators on B. 
...
\noindent\paragraph{Keeping Track of the Validator Set:} A light client verifier on A must be initialised with a committee key $\mathit{ck}_1$ 
corresponding to the genesis validator set on B with key vector $\bf{pk}_1$. At the end of each epoch on B, i.e., the time a validator set needs 
to be updated, the validators set of epoch $i$, with key vector $\bf{pk}_i$ sign a message $(i,\mathit{ck}_{i+1})$ where $\mathit{ck}_{i+1}$ as 
a committee key is also a commitment to the next epock's validator set $\bf{pk}_{i+1}$. The light client verifier on A 
...
%{\color{red}If a light client proof contains a message (signed by a committee) which is itself a commitment 
%to the public keys of a known set of validators which is incompatible with a message the same set signed on the blockchain, 
%then the signature in the proof is a valid BLS signature with the claimed set of signers and so the full node should be able to 
%report that public keys that signed both messages misbehaved.} 
...
every epoch on B, as well as, to every epoch's light client proof. Figure~\ref{figure_LC} illustrates the core accountability mechanism of our light client system. \\

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\textwidth]{LCFigure.pdf}
    \caption{Accountability mechanism: An honest full node identifies equivocating validators by combining $\mathit{proof}'_n$ for message $\mathit{info}'_n$ on B with $\mathit{proof}_n$ for a contradictory message $\mathit{info}_n$ accepted by the light client verifier on A.}
            \label{figure_LC}
\end{figure}

For short, accountability solves the following issue. Standard light client security is brittle: it is sound if and only if the home chain's honest supermajority (e.g., $>2/3$ stake) 
assumption holds. If this assumption fails, a malicious coalition can finalize a false block, which a standard light client will accept as true. 
For applications like bridges, this is catastrophic, enabling unbacked asset theft. Accountability provides a robust cryptoeconomic guarantee 
when this consensus assumption fails.The light client we construct is not responsible for consensus; it is a verifier of cryptographic evidence. 
Accountability ensures that if a light client verifier (LCV) accepts a false proof (due to a consensus failure), that proof becomes non-repudiable evidence. 
An honest full node, observing both the true state and this false proof, can use our protocol to provably identify the exact set of malicious validators who signed both. 
This mechanism enables guaranteed, targeted slashing, establishing a powerful economic deterrent: the cost of the attack (losing the slashed stake) is engineered to be 
far greater than any potential profit.  \\
...
%\item
\vspace{-0.05in}
\paragraph{Constraint System Simplicity:} Our constraint system is simple enough such that our custom SNARKs do not require a permutation argument or a matrix-vector product 
argument which general proving systems need to bind together gates. In fact, the underlying circuit for our SNARKs can be described as an affine addition gate with a couple of 
constraints added to avoid the incompleteness of our addition formulae. This simplification leads to smaller proof sizes  and faster proving times.
%\vspace{-0.2cm}
...
is used in bridges in the IBC protocol~\cite{IBC_paper}. This approach becomes prohibitively expensive for a light client verifier when processing thousands or millions of signatures.
\vspace{-0.1in}
...
In more detail, our work introduces a specialized, monolithic SNARK protocol where there commit in the CP paradigm is part of the SNARK system itself and 
written in the same arithmetisation, by design. It is crucial to contrast this with baselines built from general-purpose Commit-and-Prove (CP) compilers, 
such as Lunar~\cite{lunar} and Eclipse~\cite{eclipse}  which are modular additions to existing SNARKs like PLONK~\cite{plonk}. While general-purpose, 
these approaches are fundamentally unsuited for our application due to 
prohibitive scaling in either circuit complexity or verification cost. \\

Lunar~\cite{lunar} is a prominent CP-SNARK that, while achieving constant-time verification, is ill-suited for our application due to its $O(n)$ circuit complexity. 
A baseline using Lunar + PLONK follows the "witness-and-then-prove" paradigm. This forces the prover to supply the $n$ validator keys as a private witness, 
requiring the circuit to (1) encode the $O(n)$ aggregation logic (i.e., $n$ elliptic curve additions) and (2) use the Lunar CP-gadget to check these $O(n)$ witnesses against the public commitment. 
This resulting $O(n)$ circuit size and the prover cost are prohibitive for large validator sets. This $O(n)$ complexity is fundamental to its general-purpose model; Lunar's tools are designed to 
bind private witnesses to a commitment, but they cannot eliminate the $O(n)$ application logic (the aggregation) itself. Our protocol is a specialized, monolithic construction that merges the aggregation and commitment-binding 
into a single, holistic polynomial relation, collapsing the circuit complexity to $O(1)$. \\

Eclipse~\cite{eclipse}, another notable CP-SNARK, is fundamentally unsuited for our target application. Its primary disqualifier is the $O(\log(n))$ verification cost.  
Our use case, an on-chain light client verifier strictly demands $O(1)$ verification to ensure gas costs remain constant and independent of the validator committee size $n$. A logarithmic cost is prohibitive. 
Furthermore, like other general-purpose CP-compilers, a baseline using Eclipse would still follow the "witness-and-then-prove" paradigm, necessitating an $O(n)$ circuit to encode the aggregation logic. 
In contrast, our proposed protocol is a specialized construction that simultaneously achieves both the mandatory $O(1)$ verification and $O(1)$ circuit complexity. \\
...
%The paragraph below was a comment/was commented out when shortening the paper, but not for AC25 submission.
%\begin{comment}
 Another paradigm related to commit-and-prove is called hash-and-prove~\cite{HP_paper}: for large data structures or simply data that is expensive to be
...
the original, large, dataset. The notion of committee key scheme defined in this work has both similarities to but also differences with regard to this
