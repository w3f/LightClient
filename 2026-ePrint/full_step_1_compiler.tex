...
ranged polynomial protocols for conditional NP relations (formal definition in Appendix~\ref{supplementary_poly_protocols_appendix}) to 
...
into SNARKs $\Plastar$, and $\Pastar$, respectively. \\
We can define this compilation step for any ranged polynomial protocols. In order to do that we need: 
\begin{itemize}
...
(as used in the Definition~\ref{def_ranged_poly_protocol} in Appendix~\ref{supplementary_poly_protocols_appendix}, e.g., polynomial $aux(X)$ used in Section~\ref{sec_a}) and 
...
To conclude this step, we formally state in Appendix~\ref{supplementary_poly_protocols_appendix}, Lemma~\ref{le:compilation_step_1} under which condition and how efficiently 
