...
computed by a (possibly malicious) prover interacting with the light client verifier. Our newly introduced notion of hybrid model SNARK (see Appendix~\ref{sec:snarks_defs}) 
...
ranged polynomial protocols for conditional NP relations (formal definition in Appendix~\ref{supplementary_poly_protocols_appendix}) to hybrid 
...
\end{align*}
For completeness, we also include the full rolled out SNARK $\Pah$ for relation $\Racom$ in Appendix~\ref{sec:rolled_out} and we provide a comparison between PLONK universal 
SNARK and our custom SNARKs in Appendix~\ref{suplementary_plonk_comparison}.  
%\vspace*{-0.75cm}